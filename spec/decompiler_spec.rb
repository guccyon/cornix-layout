...
RSpec.describe Cornix::Decompiler do
  ...
  # ヘルパーメソッド: 階層構造をフラット化
  def flatten_hierarchical_mapping(mapping)
    flat = {}

    if mapping.is_a?(Hash)
      mapping.each do |key, value|
        if %w[left_hand right_hand].include?(key)
          # row0, row1, ..., thumb_keys の下のマッピングを展開
  ...
  describe 'keycode resolution' do
    describe 'QMK to alias conversion' do
      it 'converts KC_* keycodes to readable aliases' do
        ...
        layer0 = YAML.load_file("#{output_dir}/layers/0_base.yml")
        mapping = layer0['mapping']

        # 階層構造: mapping['left_hand']['row0']['tab']
        expect(mapping['left_hand']['row0']['tab']).to eq('Tab')
        expect(mapping['left_hand']['row0']['Q']).to eq('Q')
      end
      ...
      it 'converts KC_TRNS to Trans' do
        ...
        if File.exist?(layer1_path)
          ...
          # 階層構造からフラット化して探す
          flat_overrides = flatten_hierarchical_mapping(overrides)
          trans_values = flat_overrides.values.select { |v| v == 'Trans' }
      ...
      it 'converts modifier functions with KC_* arguments to simple format' do
        ...
        if File.exist?(layer1_path)
          ...
          shifted_numbers = flat_overrides.values.any? { |v| v.to_s.match?(/^LSFT\(\d+\)$/) }
      ...
      it 'preserves layer numbers in function calls' do
        ...
        # 階層構造からフラット化
        flat_mapping = flatten_hierarchical_mapping(mapping)

        # MO(3) should stay as MO(3)
        mo_values = flat_mapping.values.select { |v| v.to_s.match?(/^MO\(\d+\)$/) }
  ...
  describe 'layer extraction' do
    it 'extracts base layer with complete mapping' do
      ...
      # 階層構造の検証
      expect(mapping).to have_key('left_hand')
      expect(mapping).to have_key('right_hand')
      expect(mapping).to have_key('encoders')

      # Check thumb keys are inside left_hand/right_hand
    ...
    it 'extracts override layers with only differences' do
      ...
      if File.exist?(layer1_path)
        ...
        # 階層構造からフラット化してカウント
        flat_overrides = flatten_hierarchical_mapping(overrides)
        expect(flat_overrides.size).to be < 56 # Less than total keys
      end
    end
    ...
    it 'extracts all 10 layers correctly' do
      ...
      layer_files.each do |file|
        ...
        # ベースレイヤーは 0_base.yml、オーバーライドは N_layer.yml
        expect(basename).to match(/^\d+_(base|layer)\.yml$/)
  ...
  describe 'edge cases' do
    it 'handles missing optional fields gracefully' do
      ...
      expect(File.exist?("#{output_dir}/metadata.yaml")).to be true
      expect(File.exist?("#{output_dir}/layers/0_base.yml")).to be true
    ...
    it 'handles special characters in keycodes' do
      ...
      if File.exist?(layer1_path)
        ...
        special_chars = flat_overrides.values.select { |v| v.to_s.match?(/[`'",\/=\-]/) }
