...
4. **FileRenamer** (`lib/cornix/file_renamer.rb`)
...
5. **RenameCommand** (`lib/cornix/rename_command.rb`)
...
6. **`file_renamer_spec.rb` (44テスト)**: ファイルリネーム機能（NEW）
   - ファイルリネーム、YAML内容更新、バックアップ/ロールバック
   - バッチ処理、コンパイル検証、エラーハンドリング
   - 統合ワークフローテスト
7. `integration_spec.rb` (6テスト): Compile→Decompile→Compileのフルラウンドトリップ
