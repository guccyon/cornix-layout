...
│   ├── rename_file     # File renamer CLI (NEW)
│   └── validate        # Config validator
├── lib/cornix/
│   ├── compiler.rb
│   ├── decompiler.rb
│   ├── keycode_resolver.rb
│   ├── file_renamer.rb              # File renamer module
│   ├── rename_command.rb            # Rename command (NEW)
│   ├── keycode_aliases.yaml         # System-provided aliases (337 lines)
│   ├── position_map.yaml            # Default position map template
│   ├── position_map.rb
│   └── validator.rb
...
- **総テストケース数**: 381（Reference System実装後）
- **総行数**: 4,500行以上

**テストファイル構成**:
1. **`keycode_parser_spec.rb` (85テスト)**: KeycodeParser機能（NEW）
   - 参照形式パース（name-based, index-based）
   - 関数形式パース（シンプル、ネスト、複数引数）
   - QMKキーコード検出、レガシー形式検出、エイリアス検出
   - 引数パース（カンマ区切り、ネスト深度）
   - Unparse ラウンドトリップ検証
2. **`reference_resolver_spec.rb` (44テスト)**: ReferenceResolver機能（NEW）
   - Name→Index解決（マクロ/タップダンス/コンボ）
   - Index→Name逆解決
   - レガシー形式パススルー
   - キャッシング動作（lazy load, cache hits）
   - エラーケース（名前未発見、範囲外インデックス）
3. `compiler_spec.rb` (40テスト): Compiler機能、キーコード変換、レイヤー/マクロ/タップダンス/コンボのコンパイル
   - **+10テスト**: KeycodeParser統合、参照形式サポート
4. `decompiler_spec.rb` (37テスト): Decompiler機能、QMK→エイリアス変換、YAML生成
   - **+8テスト**: KeycodeParser統合、レガシー→name-basedアップグレード
5. **`validator_spec.rb` (65テスト)**: 設定ファイル妥当性検証
   - 既存検証: レイヤーインデックス、名前の一意性、レイヤー参照（50テスト）
   - **+15テスト**: KeycodeParser統合、参照形式検証、関数パース、キーコードパース
6. **`file_renamer_spec.rb` (57テスト)**: ファイルリネーム機能
   - ファイルリネーム、YAML内容更新、バックアップ/ロールバック（44テスト）
   - **+13テスト**: レイヤー参照自動更新（name-based, index-based, legacy処理）
7. `keycode_resolver_spec.rb` (21テスト): エイリアス⇔QMK双方向変換
8. `position_map_spec.rb` (17テスト): 物理位置マッピング
9. **`integration_spec.rb` (15テスト)**: Compile→Decompile→Compileのフルラウンドトリップ
   - 既存ラウンドトリップ（6テスト）
...
# 特定のファイル
bundle exec rspec spec/reference_resolver_spec.rb
```
